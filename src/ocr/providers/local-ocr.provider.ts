...
`;
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          reject(new InternalServerErrorException(`DeepSeek-OCR processing failed: ${stderr}`));
          return;
        }
        ...
        try {
          ...
          if (!jsonMatch) {
            reject(new InternalServerErrorException('No valid JSON output from DeepSeek-OCR'));
            return;
          }
          ...
          if (result.error) {
            reject(new InternalServerErrorException(`DeepSeek-OCR error: ${result.error}`));
            return;
          }
      ...
      pythonProcess.on('error', (error) => {
        reject(
          new InternalServerErrorException(`Failed to spawn Python process: ${error.message}`),
        );
      });
    });
  }
